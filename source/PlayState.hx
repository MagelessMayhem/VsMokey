...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['You are terrible BETCH!', 0.2], //From 0% to 19%
		...
		['OH BOY!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'whitebackground': //Week 7 - Boy, Ah hyuck
			    {
                GameOverSubstate.deathSoundName = 'scream_loss_sfx';
				GameOverSubstate.loopSoundName = 'gameOver';
				GameOverSubstate.endSoundName = 'gameOverEnd';
				GameOverSubstate.characterName = 'bf-scream-dead';
			     
				var bg:BGSprite = new BGSprite('whitebackground', -600, -200, 0.9, 0.9);
				add(bg);
                }
			case 'krimaBG':
			    {
                GameOverSubstate.deathSoundName = 'cutG_loss_sfx';
				GameOverSubstate.loopSoundName = 'gameOver';
				GameOverSubstate.endSoundName = 'gameOverEnd';
				GameOverSubstate.characterName = 'bf-cutg-dead';	

				var bg:BGSprite = new BGSprite('whitebackground', -600, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('Krima_BG', 0, 200, 0.9, 0.9);
				stageFront.updateHitbox();
				add(stageFront);
				}
			case 'WaaHooStage':
			    {
                GameOverSubstate.deathSoundName = 'wah_loss_sfx';
				GameOverSubstate.loopSoundName = 'gameOver';
				GameOverSubstate.endSoundName = 'gameOverEnd';
				GameOverSubstate.characterName = 'bf-waahoo-dead';	
			...
			case 'CreepyStage':
			    {
                GameOverSubstate.deathSoundName = 'spoopy_loss_sfx';
				GameOverSubstate.loopSoundName = 'SPOOPYgameOver';
				GameOverSubstate.endSoundName = 'gameOverEnd';
				GameOverSubstate.characterName = 'bf-spoopy-death';	

				var bg:BGSprite = new BGSprite('spoopysky', -550, -200, 0.7, 0.7);
				add(bg);

				var stageFront:BGSprite = new BGSprite('spoopyfloor', -800, 0, 1, 1);
				stageFront.updateHitbox();
				add(stageFront);				
				}
			case 'KrampusStage':
				...
				forest = new BGSprite('Forest', -800, -250, 1, 1);
				forest.updateHitbox();
				add(forest);			

				var fight = new FlxSprite(0, 250);
                fight.frames = Paths.getSparrowAtlas('EpicKrimaFight_BG');
                fight.animation.addByPrefix('play', 'KrimaFight', 24, true);
                fight.animation.play('play');
                fight.antialiasing = ClientPrefs.globalAntialiasing;
                fight.updateHitbox();
                add(fight);	

                blackvoid = new BGSprite('BlackVoidLOL', -800 -250, 1, 1);
			...
			case 'WindowsStage':
				...
				GameOverSubstate.loopSoundName = 'gameOver-pixel';
				GameOverSubstate.endSoundName = 'gameOverEnd-pixel';
				GameOverSubstate.characterName = 'bf-pixel-death';	

				var windowslol:BGSprite = new BGSprite('windows_xp_bg', -900, -300, 1, 1);
				add(windowslol);			
				}					
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
			    ...
			    case 'boy':
				    startVideo('mokeycutscene');

			    case 'ah-hyuck':
				    startVideo('groobycutsceneSUS');	
							...
							if(!endingSong) {
								...
								if(!daNote.ignoreNote){
										    switch(daNote.noteType){
			                                    case 'Momi Bullet Note':{
				                                health = 0;
				                                return;
			                                }
												case 'Tough Attack Note':{
				                                health = 0;
				                                return;
			                                }											
		                                }
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			case 'Change Position':{
				...
				char.x = Std.parseFloat(split[0]);
				char.y = Std.parseFloat(split[1]);
			}
			case 'Black':{
				FlxTween.tween(blackScreen, {alpha: Std.parseFloat(value1)}, Std.parseFloat(value2));
			}
			case 'Black Void Appear':{
				var value:Int = Std.parseInt(value1);
				if(value != 0){
					blackvoid.visible = true;
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					FlxG.sound.playMusic(Paths.music('lolMenu'));
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			switch(note.noteType) {
				case 'Instakill Note': //Instakill Note
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							if(!note.isSustainNote) {
								health -= 3.26; //3.26 + 0.04 = -0.3 (-15%) of HP if you hit a hurt note
								if(!note.noteSplashDisabled) {
									spawnNoteSplashOnNote(note);
								}
							}
							else health -= 3.06; //3.06 + 0.04 = -0.1 (-5%) of HP if you hit a hurt sustain note
			...
			if(note.noteType == 'Tough Attack Note') {
				if(boyfriend.animOffsets.exists('dodge')) {
					boyfriend.playAnim('dodge', true);
					boyfriend.specialAnim = true;
					boyfriend.heyTimer = 0.1;
